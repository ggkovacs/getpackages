            ...
            for (var j = 0, l = item.sources.length; j < l; j++) {
                ...
                if (data.scriptsSourcePathBeforeTranspiling.indexOf(dirname) === -1) {
                    ...
                    data.scriptsToTranspiling.push({
                        module: currentItem.module,
                        package: currentItem.package || null,
                        base: currentItem.sources,
                        sources: dirname,
                        dest: path.join(currentItem.sources, currentItem.transpiledjs)
                    });
                }
            }

            bundle.transpiledSources = sources;
        } else {
            sources = item.sources;
        }
...
function buildFromExtraParams(currentItem) {
    ...
    //         data.customPaths[key] = {
    //             sources: path.join(currentItem.sources, item.sources)
    //         };
    //     }
